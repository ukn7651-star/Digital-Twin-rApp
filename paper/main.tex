...
to idle. \phold{with the full-stack integration (multi-UE Standalone core, FlexRIC
KPM, and the rApp driving OAI in the loop), we further report real per-UE iperf3
goodput of \tbd{} and a twin-predicted-vs-stack-measured rApp gain gap of \tbd{}.}
The platform runs on commodity CPU hardware and is released open-source.
\end{abstract}
...
a twin-to-OAI inter-cell-interference noise-floor bridge.
\item \phold{a full-stack closed loop---multi-UE Standalone, FlexRIC KPM, and the
rApp acting on OAI---with real per-UE goodput and a twin-to-stack fidelity gap
(\tbd{}).}
...
stack grounded per link; full rApp-driven multi-cell OAI is the in-progress
integration reported in Sec.~\ref{sec:results}-E.
...
\phold{Full-stack setup (in progress): OAI 5G Core (oai-cn5g, Docker) + gNB (SA) +
$N$ UEs in network namespaces, each over its ray-traced channel and interference
noise floor; FlexRIC (E2SM-KPM) for KPI streaming; iperf3 per UE for goodput; the
rApp issues CIO/handover actions over O1/telnet. Scale: \tbd{} UEs.}
...
not an absolute level change; absolute goodput is the object of the full-stack
Standalone runs in Sec.~\ref{sec:results}-E.

\subsection{Full-stack integration (in progress)}
\phold{the following results are filled once the full-stack closed loop lands.}

\textit{E.1 Real per-UE goodput (multi-UE SA + iperf3).}
\phold{per-UE downlink iperf3 goodput vs.\ the twin's predicted throughput.}
\begin{table}[h]
\centering \footnotesize
\begin{tabular}{@{}lccc@{}}
\toprule
UE & twin-pred.\ (Mbps) & OAI iperf3 (Mbps) & rel.\ err. \\
\midrule
\tbd{} & \tbd{} & \tbd{} & \tbd{} \\
\bottomrule
\end{tabular}
...
\textit{E.2 FlexRIC KPM monitoring.} \phold{per-UE MCS/BLER/RSRP/throughput
streamed via E2SM-KPM during the run.}

\textit{E.3 Closed-loop rApp on OAI---fidelity gap.} \phold{run the rApp against the
live multi-cell OAI stack; report twin-predicted vs.\ stack-measured gain.}
\begin{table}[h]
\centering \footnotesize
\begin{tabular}{@{}lccc@{}}
\toprule
Metric & twin-pred. & OAI-meas. & gap \\
\midrule
median throughput   & $+9.5\%$      & \tbd{} & \tbd{} \\
served cell-edge    & $+22.4\%$     & \tbd{} & \tbd{} \\
peak-load reduction & $-0.75$ UEs   & \tbd{} & \tbd{} \\
\bottomrule
\end{tabular}
\end{table}
...
system. Section~\ref{sec:results}-E reports this integration as it lands; the two
bridges (channel injection and interference noise floor) are the mechanism that
connects them. (2)~Inter-cell interference is modelled as (frequency-selective)
...
(Sec.~\ref{sec:results}-E) is the in-progress integration that makes it the first
