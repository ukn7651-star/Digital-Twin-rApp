%% IEEE conference paper -- compile with pdfLaTeX (e.g. on Overleaf).
%% Upload this file together with the three fig_*.png in this folder.
%% (It also compiles without them, via \IfFileExists.)
...
\newcommand{\n}{$\times$}
% include a figure, or a labeled box if the file is absent
...
sweep shows per-user throughput rising $1.7$--$1.8\times$ as inter-cell load falls
to idle. We outline the path to a fully integrated multi-UE closed loop, and release
the platform, harness, and results as open source; everything runs on commodity CPU
hardware.
\end{abstract}
...
(PHY) layer~\cite{ns3sionna}; OAI-based emulators execute the real 5G-NR
stack~\cite{oai} but, with realistic propagation, cover a single link; commercial
twins close the rApp loop but are proprietary.
...
closed control loop---and no open platform combines them. We contribute such a
platform---integrating the four as components---with a statistically rigorous,
honest evaluation: the multi-cell twin and its rApp run today, the real stack is
grounded per link, and the remaining full-stack loop is designed end-to-end
(Sec.~\ref{subsec:fullstack}). Our contributions are:
\begin{enumerate}
...
\item A reproducible experiment harness and results, released as open source.
\end{enumerate}
...
step in Sec.~\ref{subsec:fullstack}.
...
Two design choices distinguish our modelling from standard system-level
practice~\cite{eesm}. First, the SINR-to-MCS lookup is measured from OAI's own PHY
(\texttt{nr\_dlsim}) rather than from idealised curves, so the network-scale KPIs
inherit a real receiver's implementation losses. Second, our multi-cell
interference-as-noise-floor bridge is a site-specific generalisation of the 3GPP
OFDMA channel noise generator (OCNG)~\cite{ocng}: instead of loading unused
resources with a generic pattern, we derive each UE's interference from the
ray-traced neighbour channels.
...
\resizebox{\textwidth}{!}{%
\begin{tikzpicture}[
  font=\small, >=Latex, node distance=6mm and 10mm,
  box/.style={draw,rounded corners,align=center,inner sep=4pt,minimum height=9mm,fill=blue!5},
  dat/.style={draw,align=center,inner sep=4pt,minimum height=9mm,fill=black!5},
  oai/.style={draw,rounded corners,align=center,inner sep=4pt,minimum height=9mm,fill=orange!15}]
  \node[dat] (osm) {OpenStreetMap\\bbox};
  \node[dat,above=of osm] (net) {Network source\\(random / OpenCelliD)};
  \node[box,right=of osm] (scene) {3D scene\\(Mitsuba)};
  \node[box,right=of scene] (rt) {Sionna RT\\ray tracing};
  \node[dat,right=of rt] (cfr) {ray-traced\\CFR};
  \node[oai,below=14mm of rt] (dlsim) {OAI \texttt{nr\_dlsim}\\(offline SINR$\to$MCS curve)};
  % fast analytical layer (top branch)
  \node[box,right=18mm of cfr,yshift=10mm] (kpi) {KPI engine:\\per-RE SINR (MRT)+EESM\\$\to$ OAI-measured MCS};
  \node[box,right=of kpi] (rapp) {Traffic-steering\\rApp (CIO)};
  \node[dat,right=of rapp] (kout) {per-UE / per-cell\\throughput};
  % real-stack grounding (bottom branch)
  \node[oai,right=18mm of cfr,yshift=-14mm] (oai) {OAI gNB--UE link\\(rfsim, injected channel)};
  \node[oai,right=of oai] (floor) {per-UE interference\\noise-floor bridge};
  % arrows
  \draw[->] (net) -- (scene); \draw[->] (osm) -- (scene);
  \draw[->] (scene) -- (rt);  \draw[->] (rt) -- (cfr);
  \draw[->] (cfr.east) -- (kpi.west); \draw[->] (kpi) -- (rapp); \draw[->] (rapp) -- (kout);
  \draw[->] (cfr.east) -- (oai.west);
  \draw[->] (dlsim.east) -| ([xshift=9mm]cfr.east) |- ([yshift=-2mm]kpi.west);
  \draw[->] (floor) -- (oai);
  % layer captions
  \node[font=\itshape\footnotesize, above=1mm of kpi] {fast analytical layer (all cells / UEs)};
  \node[font=\itshape\footnotesize, below=1mm of oai] {real-stack grounding (sampled links)};
\end{tikzpicture}}
...
\textbf{Channel.} Sionna~RT~\cite{sionnart} places one transmitter per cell (3GPP sector pattern,
...
seeds. \textbf{Random layouts.} Eight seeded layouts per scene---24 runs
($323$\,s on one CPU server). \textbf{Configuration sweep.} Neighbour load factor
in $\{0,0.25,0.5,0.75,1.0\}$ on a cached channel per scene. Radio and topology
...
Across 24 runs, relative to strongest-cell association
(Tables~\ref{tab:results}--\ref{tab:abs}, Fig.~\ref{fig:cdf}), the rApp helps
exactly the users a strongest-cell policy under-serves---improving the median,
cell-edge, and fairness metrics---while shedding peak load. In absolute terms the
overall median per-UE rate rises from $8.5$ to $9.2$\,Mbps and the served cell-edge
rate from $3.5$ to $4.2$\,Mbps, whereas the mean is essentially unchanged
($11.4$\,Mbps). Three honest observations follow: aggregate rate barely moves
($+0.5\%$; the proportional-fair objective trades sum for fairness), outage is
unchanged (steering relocates covered users, it does not create coverage), and
variance is high (layout-dependent)---motivating multi-seed evaluation. The
controller itself is inexpensive: the coordinate ascent reaches a local optimum in
a few per-cell line-search sweeps, keeping the loop well within an rApp's
non-real-time (seconds-and-above) budget.

The per-scene breakdown is instructive (Fig.~\ref{fig:gains}). Berlin, the densest
scene, shows the largest served cell-edge gain ($+41\%$): dense blockage produces
stronger load imbalance for the rApp to exploit. Paris shows the smallest gain
($+6\%$) and the highest outage ($23.8\%$): when many users lack coverage entirely,
...
\begin{table}[t]
\caption{Absolute per-UE throughput, baseline$\to$rApp
(Mbps, mean over seeds).}
\label{tab:abs}
\centering
\footnotesize
\begin{tabular}{@{}lcccc@{}}
...
\caption{rApp gains over strongest-cell association by scene (mean $\pm$ std over
eight seeds). The direction is consistent; the magnitude is deployment-dependent,
with the widest spread on the served cell-edge.}
\label{fig:gains}
\end{figure}
...
\caption{UE outage vs.\ inter-cell load factor, for one cached layout per scene.}
\label{fig:outage}
\end{figure}

\subsection{Sensitivity to inter-cell load}
As the neighbour load factor falls from $1.0$ to $0$, mean per-UE throughput rises
$1.7$--$1.8\times$ (Berlin $13.6\to22.7$, Paris $14.9\to25.2$, Manhattan
...
\caption{Mean per-UE throughput vs.\ inter-cell load factor, for one cached layout
per scene.}
\label{fig:tput}
\end{figure}
...
path outlined in Sec.~\ref{subsec:fullstack}.

\subsection{Toward full-stack integration}\label{subsec:fullstack}
...
ray-traced channel and computed interference noise floor; FlexRIC~\cite{flexric}
would stream per-UE KPMs (E2SM-KPM); iperf3 would supply real per-UE goodput; and the
...
and the full-stack integration of Sec.~\ref{subsec:fullstack} can be completed on a
...
system. Section~\ref{subsec:fullstack} outlines this integration; the two bridges
...
(Sec.~\ref{subsec:fullstack}) is the planned integration that will make it the first
