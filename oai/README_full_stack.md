...
> with root and Docker**. They are **NOT verified in the Cloud sandbox**, which
> has no Docker and no `ip`/network-namespaces. Everything on the *twin* side
> (channel export, interference-floor computation, rApp decision) is pure Python
> and unit-tested and runs anywhere; the OAI/Docker/FlexRIC steps are validated
> on a host. The single-UE phy-test path in `oai/README.md` **is** runnable in a
> CPU sandbox and was used to verify the RT channel injection.

## What is verified where

| Component | Script / module | Verified in sandbox? |
|---|---|---|
