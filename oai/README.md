...
2. **Run OAI as a NON-root user** — under `sudo`, OAI tries `SCHED_FIFO`, which
   some restricted containers forbid (crash); as a normal user it skips real-time scheduling.
...
- Bare `phy-test` has no user traffic, so `goodput` reads 0 while SNR/MCS/BLER are
  ...
  (needs `CAP_NET_ADMIN`, which some environments may restrict).
- The OAI build (~1.6 GB) is not in the repo; for a persistent deployment, run
  `setup_oai.sh` once as part of your host provisioning.
