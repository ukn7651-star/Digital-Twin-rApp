...
Digital twins are increasingly used to develop and validate O-RAN control applications before deployment. Existing open twins, however, each trade off one capability. The Open Wireless Digital Twin (OWDT) pairs OpenAirInterface (OAI) with Sionna RT for a faithful single-cell, single-user link but only monitors KPIs. Tiny-Twin runs a real OAI stack for multiple users on a CPU with a control loop, but on a single cell and from replayed channel traces. The Orange RIC-TaaP / ns-O-RAN effort closes rApp loops across many cells with Sionna RT, but on ns-3's abstracted physical layer. OpenTwin similarly closes an xApp/rApp control loop and even auto-corrects KPM deviations between the twin and the live network, but is likewise built on the ns-O-RAN abstract physical layer rather than a real stack or a site-specific ray-traced channel. Recent OAI+FlexRIC demonstrations close handover loops across real cells, but over the air, without a site-specific ray-traced channel. Within OAI itself, an emerging ray-tracing channel emulator with a Godot front-end and REST scene control (built on Sionna RT) feeds the virtual-time radio simulator (VRTSim), giving an official path to drive a real OAI link over a ray-traced channel; it targets the per-link channel-emulation component (multi-RU/UE support is on its roadmap) rather than a multi-cell twin with a closed control loop, and is a natural backend our injection step can adopt as it matures. None of these combines site-specific ray tracing, a multi-cell/multi-user network, a physical-layer-grounded link model, and a closed control loop in one open, CPU-only platform.
...
Sionna RT (arXiv:2303.11103); OpenAirInterface (Computer Networks 2025); FlexRIC (CoNEXT '21); OWDT (arXiv:2503.12177 / IEEE Access 2025); Tiny-Twin (arXiv:2601.08217); OpenTwin (arXiv:2605.24662); Orange RIC-TaaP / ns-O-RAN-flexric; OAI ray-tracing channel emulator + VRTSim (Eurecom GitLab, gitlab.eurecom.fr/oai/raytracing-channel-emulator; OAI Kista workshop 2025); OAI+FlexRIC customized handover (ACM 2025); ns-3 + Sionna RT (arXiv:2501.00372); Rimedo Labs O-RAN whitepapers (traffic steering); 3GPP TS 38.214; 3GPP TS 38.106 (OCNG).
